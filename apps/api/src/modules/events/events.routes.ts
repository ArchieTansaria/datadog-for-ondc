...
export const eventRoutes: FastifyPluginAsync = async (fastify: FastifyInstance) => {
  fastify.post('/webhook', async (request, reply) => {
    ...
    if (!order) {
      ...
      const isValid = stateMachineService.isValidTransition(order.currentState, targetState);
      
      if (isValid) {
        // Update Order State
        order = await ordersRepository.updateState(order.id, tenantId, targetState);
      } else {
    ...
    await eventsRepository.create({
      ...
      eventType: targetState, // Storing target state as internal event type
